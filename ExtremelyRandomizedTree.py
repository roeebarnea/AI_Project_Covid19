...
def extra_trees(WWC, crit, min_sample_split, estim):
    X, Y = take_X_Y(WWC)
    X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=0.20)
    decision_tree = ExtraTreesClassifier(criterion=crit, min_samples_split=min_sample_split, max_features=1.0, n_estimators= estim)
...
def extra_trees_couple_of_tests(WWC, crit, min_sample_split, estim, tests):
    scores = []
    for i in range(tests):
        scores.append(extra_trees(WWC, crit, min_sample_split, estim))

    # print(scores)
    # print(sum(scores) / len(scores))
    return sum(scores) / len(scores)

def ExtremlyRandomizedTree_Exp(file_name):
    ...
    for n in n_estimators:
        ...
        for ss in sample_splits:
            scores_row.append(extra_trees_couple_of_tests(WWC, 'gini', ss, n, TESTS))
        print(scores_row)
    ...
    for n in n_estimators:
        ...
        scores_row = []
        for ss in sample_splits:
            scores_row.append(extra_trees_couple_of_tests(WWC, 'entropy', ss, n, TESTS))
        print(scores_row)
        mat.append(scores_row)
    print()
    print()
